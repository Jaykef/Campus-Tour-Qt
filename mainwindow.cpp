...
void MainWindow::paintEvent(QPaintEvent *event)
{
    ...
    QPixmap menu(":/images/menu.png");              // 读取菜单图片

    QRect target2(200, 0, 600, 600);                // 设置校园地图绘制位置
    QPixmap umap(":/images/map.png");    // 读取校园地图图片
...
void MainWindow::on_pushButton3_clicked()
{
    ...
    QPixmap *img = new QPixmap;
    img->load(":/images/map.png");
    QLabel *label = messageWindow->findChild<QLabel *>("label");

    label->setPixmap(*img);
    textBrowser->setText(tr("北京工业大学，简称xx，位于xx省xx市，是国家中西部高校基础能力建设工程重点支持高校"
...
void MainWindow::path(mgraph c, int m, int n, int k)
{
    ...
    if (d[k] == n && k<8) {      // 若d[k]是终点n且景点个数<=8,则输出该路径
        ...
        if (length <= 250) {        // 打印路径长度小于定长的路径
            ...
            ui->textBrowser->append("总时间" + QString::number(length/20) + "mins") ;
        }
...
void MainWindow::shortdistance(mgraph c, int start, int end)
{
    ...
    ui->textBrowser2->append("总时间" + QString::number(shortest[i][j]/15)+"mins");
